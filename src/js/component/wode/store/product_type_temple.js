...
export default class StoreManageTemple extends React.Component{
    ...
    render(){
            ...
            _data.map(obj=>
                <li className="modal-content-radio-item">
                    <label htmlFor={obj.id}>
                        <span>{obj.name}</span>
                        <input id={obj.id} type="checkbox" name="sy" value={obj.id} checked={obj['checkArr']}/>
        ...
        return (
            <div>
                <header className="myHeader"><a className="btn-back" id="javascript:;"><img src={back}/></a>在售产品系列</header>
                <div>
                    {child}
                </div>
